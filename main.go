...
func main() {
	// Compile shaders if called
	if len(os.Args) == 2 {
		if os.Args[1] != "--compile" && os.Args[1] != "-c" {
			goto main
		}
		...
		if len(a) < 2 {
			log.Fatalln("Missing compile dir")
		}
...
main:
	...
	var windowWidth = mode.Width
	var windowHeight = mode.Height
	...
	fmt.Println("Screen", windowWidth, windowHeight)

	// Configure the vertex and fragment shaders
	program, err := newProgram(util.BasicGeom, util.BasicVert, util.BasicFrag)
	if err != nil {
		panic(err)
	}
...
func newProgram(geometryShaderSource, vertexShaderSource, fragmentShaderSource string) (uint32, error) {
